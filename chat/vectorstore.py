...
class VectorStoreMethods:
    ...
    def update_vectorstore(cls:'Bot'):
        ...
        metadata = MetaData()
        activity_table = Table("activity", metadata, autoload_with=engine)

        stmt = select(activity_table.c.activity_id)
        ...
        # 결과 추출
        activity_ids:set[str] = {row[0] for row in results}

        ids_to_add:list[str] = list(activity_ids - ids_in_vectorstore)
        ids_to_delete:list[str] = list(ids_in_vectorstore - activity_ids)
        ...
        SELECT `activity_id`, `activity_name`, `activity_content`, `activity_type`, `keyword`, `site_url`, `start_date`, `end_date`
        FROM activity WHERE `activity_id` IN ({",".join(placeholders)});""",
